...
const MovieList = () => {
  ...
  const [star, setStar] = useState(
    JSON.parse(localStorage.getItem('idsAndStars')) ? JSON.parse(localStorage.getItem('idsAndStars')) : []
  )
              ...
              <ul className="movies">
                <>
                  {allFilms.map((searchMovie, index) => {
                    return <MovieCard key={index} {...searchMovie} star={star} setStar={setStar} />
                  })}
                </>
              </ul>

              <Page datas={datas} getAllMovies={getAllMovies} className="stars" />
            </>
          )}
