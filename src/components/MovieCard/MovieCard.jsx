...
const Card = ({
  ...
  const valueFromState = star.filter((item) => item.id === id)

  const onRateSetStars = (num) => {
    localStorage.setItem('idsAndStars', JSON.stringify([...star, { id, num }]))
    setStar([...star, { id, num }])
    ...
    <div className="movies__card">
      ...
      <div className="movies__description">
        ...
        <Rate
          ...
          defaultValue={0}
          count={10}
          onChange={onRateSetStars}
          className="movies__stars-yellow"
          value={valueFromState.length > 0 ? valueFromState[0].num : 0}
        />
      </div>
    </div>
  )
